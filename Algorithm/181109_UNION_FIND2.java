...
public class UNION_FIND2 {
	...
	public boolean find(char v, char u) { // v와 u가 cycle인지 아닌지
		int vNum = -1, uNum = -1;
		...
		// v의 root 를 찾는다.
		while(PARENT[vNum] > -1) vNum = PARENT[vNum];// 빈칸을 채우시오: v의 root를 찾는다.

		// u의 root 를 찾는다.
		while(PARENT[uNum] > -1) uNum = PARENT[uNum];// 빈칸을 채우시오: u의 root를 찾는다.
